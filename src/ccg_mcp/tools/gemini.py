...
Gemini CLI 作为一个强大的子代理（Sub-agent），由主架构师（Gemini）按场景控制。
"""
...
def run_gemini_command(
    ...
    def is_turn_completed(line: str) -> bool:
        ...
        try:
            ...
            # Gemini CLI 使用 turn.completed 表示回合完成，result 表示执行结束
...
async def gemini_tool(
    ...
    **重试策略**：默认允许 1 次重试
    ...
    # 关键修改：为了确保进入 Headless (One-shot) 模式，必须提供一个 positional argument。
    # 真正的 PROMPT 内容通过 stdin 传递（以支持长文本），这里传入一个占位符。
    # Gemini 会将 stdin 的内容追加到这个 query 后面。
    cmd.append("Please execute the instructions provided via stdin.")
