...
def build_claude_env(config: dict[str, Any]) -> dict[str, str]:
    """构建 Claude 工具调用所需的环境变量
    ...
    model = claude_config.get("model", "claude-3-5-sonnet-latest")

    env = os.environ.copy()

    # API 认证
    if claude_config.get("api_token"):
        env["ANTHROPIC_AUTH_TOKEN"] = claude_config["api_token"]
    
    if claude_config.get("base_url"):
        env["ANTHROPIC_BASE_URL"] = claude_config["base_url"]
    ...
    # 用户自定义的额外环境变量
    for key, value in claude_config.get("env", {}).items():
        env[key] = str(value)
