...
Empower **Gemini** as the architect to orchestrate **Coder** for code execution, **Codex** for code quality review, and **Claude** for expert consultation,<br>forming an **automated multi-party collaboration loop**.
...
> 💡 **Attribution**: This project is based on [Coder-Codex-Gemini](https://github.com/FredericMN/Coder-Codex-Gemini).
...
| :--- | :--- |
| **🧠 Cost Optimization** | **Gemini** handles high-intelligence reasoning & orchestration (expensive but powerful), while **Coder** handles heavy lifting of code execution (cost-effective volume). |
| **🧩 Complementary Capabilities** | **Gemini** compensates for **Coder**'s creativity gaps, **Codex** provides an independent third-party review perspective, and **Claude** offers diverse expert opinions. |
| **🛡️ Quality Assurance** | Introduces a dual-review mechanism: **Gemini Acceptance** + **Codex Final Review** to ensure code robustness. |
...
*   **Gemini**: 👑 **Architect / Coordinator**
    *   Responsible for requirement analysis, task decomposition, prompt optimization, guiding Coder, and final decision-making.
*   **Coder**: 🔨 **Executor**
...
*   **Claude**: 🧠 **Versatile Expert (Optional)**
    *   A top-tier AI expert on par with Gemini. Can serve as senior consultant, independent reviewer, or code executor. Invoked on-demand.

### 📊 Case Study

**[Unit Test Batch Generation](cases/2025-01-05-unit-test-generation/README.md)** - CCG Architecture Real-World Test

| Metric | Pure Gemini Approach | CCG Collaborative Approach | Notes |
| :--- | :--- | :--- | :--- |
...
| **Gemini Cost** | $3.13 | $0.29 | **91% savings** (architecture orchestration only) |
| **Coder Cost** | $0 | $0.26 | Handles heavy code generation tasks |
| **Quality Review** | ❌ No independent review | ✅ Gemini Acceptance + Codex Final Review | Dual quality gates, controllable code quality |

**Key Advantages**:
- 💰 **Cost Optimization**: Gemini outputs only concise instructions, leveraging cheap input pricing for review work, avoiding expensive code token output
...
- 🛡️ **Quality Assurance**: Dual-review mechanism (Gemini Acceptance + Codex Final Review), controllable code quality
...
flowchart TB
    ...
    subgraph GeminiLayer ["Gemini - Architect"]
        Gemini["🧠 Analysis & Decomposition"]
    ...
    subgraph ToolLayer ["Execution Layer"]
        ...
        GeminiTool["🧠 Gemini Tool<br><code>gemini CLI</code><br>sandbox: workspace-write"]
    end
    ...
    MCP -->|"Stream JSON"| GeminiTool

    Coder -->|"SESSION_ID + result"| Review
    GeminiTool -->|"SESSION_ID + result"| Review
    ...
    Review -->|"⚠️ Minor Optimization"| Gemini
```
...
1. User submits a requirement
       ↓
2. Gemini analyzes requirements, decomposes tasks, constructs precise Prompt
       ↓
3. Calls coder (or gemini) tool → Execute code generation/modification
       ↓
4. Gemini accepts results, consults Claude if needed
       ↓
5. Calls codex tool → Independent Code Review (submitted after Gemini acceptance)
       ↓
...
The Skills layer provides workflow guidance to ensure Gemini uses MCP tools correctly.
...
Add mandatory rules to `~/.claude/CLAUDE.md` to ensure Gemini follows the collaboration workflow:
...
**Gemini is the final decision maker**. All AI opinions are for reference only; think critically to make optimal decisions.

## Role Distribution

| Role | Position | Purpose | sandbox | Retry |
|------|----------|---------|---------|-------|
| **Gemini** | Architect/Acceptance/Final Decision | Requirement analysis, task planning, guide Coder | workspace-write | No retry by default |
...
| **Claude** | Senior Consultant (On-demand) | Architecture design, second opinion, frontend/UI | workspace-write (yolo) | 1 retry by default |

## Core Workflow

1. **Gemini Plans**: Analyze requirements, decompose tasks, define acceptance criteria
2. **Coder Executes**: Follow Gemini guidance to implement changes
3. **Gemini Accepts**: Quick check after Coder completes; Gemini fixes issues directly
4. **Codex Reviews**: Call review after milestone development; if issues found, delegate to Coder for fixes, iterate until passed
...
- **Precise Prompt**: Clear goal, sufficient context, explicit acceptance criteria (produced by Gemini)
- **Modular Split**: Related changes can be combined; independent modules separated
- **Phased Review**: Gemini verifies each module; Codex reviews at milestones

## Pre-coding Preparation (Complex Tasks)

1. Search for affected symbols/entry points
2. List files that need modification
3. For complex issues, consult with Codex or Claude first

## Claude Trigger Scenarios

- **User Explicit Request**: User specifies using Claude
- **Gemini Autonomous Call**: When designing frontend/UI, or need second opinion/independent perspective
```
...
- Gemini needs a second opinion or independent perspective
...
| **Global Prompt Layer** | Mandatory rules (ensure Gemini follows collaboration workflow) | Fixed (~20 lines) | Recommended |

**Why is complete configuration recommended?**
- **Pure MCP**: Tools available, but Gemini may not understand when/how to use them
- **+ Skills**: Gemini learns the workflow, knows when to trigger collaboration
- **+ Global Prompt**: Mandatory rules ensure Gemini always follows collaboration discipline
