...
一个统一的 MCP 服务器，让 Gemini 作为架构师调度 Coder 执行代码任务、Codex 审核代码质量、Claude 提供专家咨询，形成自动化的多方协作闭环。

## 核心价值

| 维度 | 价值 |
|------|------|
| **成本优化** | Gemini 负责思考（贵但强），Coder 负责执行（量大管饱） |
| **能力互补** | Gemini 补足 Coder 创造力短板，Codex 提供独立审核视角，Claude 提供多元化专家意见 |
| **质量保障** | 双重审核机制（Gemini 验收 + Codex 终审） |
| **全自动闭环** | 拆解 → 执行 → 审核 → 重试，无需人工干预 |

## 角色分工

```
Gemini            →  架构师 + 验收者 + 最终决策者 + 协调者
Coder (可配置)    →  代码执行者（生成、修改、批量任务）
Codex (OpenAI)    →  独立代码审核者（质量把关）
Claude (可选)     →  多面手专家（架构设计、第二意见、前端/UI）
```
