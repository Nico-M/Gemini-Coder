...
让 **Gemini** 作为架构师调度 **Coder** 执行代码任务、**Codex** 审核代码质量，**Claude** 提供专家咨询，<br>形成**自动化的多方协作闭环**。
...
> 💡 **致谢**: 本项目基于 [Coder-Codex-Gemini](https://github.com/FredericMN/Coder-Codex-Gemini) 进行二次开发。
...
| 维度 | 价值说明 |
| :--- | :--- |
| **🧠 成本优化** | **Gemini** 负责高智商思考与调度（贵但强），**Coder** 负责繁重的代码执行（量大管饱）。 |
| **🧩 能力互补** | **Gemini** 补足 **Coder** 的创造力短板，**Codex** 提供独立的第三方审核视角，**Claude** 提供多元化专家意见。 |
| **🛡️ 质量保障** | 引入双重审核机制：**Gemini 验收** + **Codex 终审**，确保代码健壮性。 |
...
*   **Gemini**: 👑 **架构师 / 协调者**
    *   负责需求分析、任务拆解、Prompt 优化、指导 Coder 以及最终决策。
*   **Coder**: 🔨 **执行者**
...
flowchart TB
    ...
    subgraph GeminiLayer ["Gemini - 架构师"]
        Gemini["🧠 需求分析 & 任务拆解"]
        Prompt["📝 构造精确 Prompt"]
        Review["🔍 结果审查 & 决策"]
    end
    ...
    subgraph ToolLayer ["执行层"]
        ...
        GeminiTool["🧠 Gemini 工具<br><code>gemini CLI</code><br>sandbox: workspace-write"]
    end
...
1. 用户提出需求
       ↓
2. Gemini 分析需求、拆解任务，构造精确 Prompt
       ↓
3. 调用 coder (或 gemini) 工具 → 执行代码生成/修改
       ↓
4. Gemini 验收结果，必要时先咨询 Claude
       ↓
5. 调用 codex 工具 → 独立 Code Review（Gemini 验收后提交）
       ↓
6. 根据审核结论：通过 / 优化 / 重新执行
```
...
Skills 层提供工作流指导，确保 Gemini 正确使用 MCP 工具。
...
在 `~/.claude/CLAUDE.md` 中添加强制规则，确保 Gemini 遵守协作流程：
...
| **Claude** | 高阶顾问（按需） | 架构设计、第二意见、前端/UI | workspace-write (yolo) | 默认 1 次 |

## 核心流程

1. **Gemini 规划**：分析需求、拆解任务、给出指导与验收标准
...
- **精准 Prompt**：目标明确、上下文充分、验收标准清晰（由 Gemini 输出）
...
- **Gemini 自主调用**：设计前端/UI、需要第二意见或独立视角时
```
...
| **全局 Prompt 层** | 强制规则（确保 Gemini 遵守协作流程） | 固定（约 20 行） | 推荐 |

**为什么推荐完整配置？**
- **纯 MCP**：工具可用，但 Gemini 可能不理解何时/如何使用
- **+ Skills**：Gemini 学会工作流程，知道何时触发协作
- **+ 全局 Prompt**：强制规则确保 Gemini 始终遵守协作纪律
